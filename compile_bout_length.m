close all;
load('bout_matches_ALL_all_thres0.1.mat', 'bout_matches_all');
load('FLYMAT_HumanAnnotation_v3.mat');
load('common-params-annot-analysis.mat');

human_annot_scores = 0:6; 
wingext_bin_edges = [0,6,30,60:60:300,600];
...
for i=1:length(behav_list)
    ...
    saveas(double(gcf), sprintf('human_annotated_bout_length_intersection_histogram-%s.eps', behav), 'eps');
    ...
    saveas(double(gcf), sprintf('human_annotated_bout_length_union_histogram-%s.eps', behav), 'eps');
    ...
    saveas(double(gcf), sprintf('jaaba_bout_length_histogram-%s.eps', behav), 'eps');
    ...
    saveas(double(gcf), sprintf('human_annotated_bout_length_intersection_disttribution-%s.eps', behav), 'eps');
    ...
    if i == 2
        ...
        max_length = max(horzcat(annot_union_bout_length_binned{:}));
        histcount_cell = cellfun(@(lengths) histcounts(lengths, 'BinMethod', 'integers', 'BinLimits', [min_length, max_length]), annot_intsct_bout_length_binned, 'UniformOutput', false);
    end
    area(vertcat(histcount_cell{:}), 'FaceColor', 'flat');
    xticklabels(human_annot_scores);
    ...
    saveas(double(gcf), sprintf('human_annotated_bout_length_union_disttribution-%s.eps', behav), 'eps');
    ...
    if i == 2
        ...
        max_length = max(horzcat(jaaba_bout_length_binned{:}));
    ...
    saveas(double(gcf), sprintf('jaaba_annotated_bout_length_disttribution-%s.eps', behav), 'eps');
